...
board_deploy_chk()
{
	...
	if [ x"$power_control_info" = x"" ]; then
		echo "Power$board_no is not deployed into Open Lab, please touch the lab manager." >&2
		return 1
	fi
...
{
	local board_no=$1
	board_file_prepare_ex $board_no
}

#############################################################################
# gen_lock_file usr brdNo
#############################################################################
gen_lock_file()
{
	local user=$1
...
{
	local board_no=$1
	if (find_lock_file $board_no >/dev/null); then
		return 0
	else
		return 1
	fi
...
board_using_chk_ex()
{
	...
	local email=
	local curUser=$(get_board_current_user $board_no)

	if [ x"$curUser" = x"" ]; then
		# the board is currently not using by anyone
		if !(board_is_lock $board_no); then
			return 0
		fi

		# the board is locked
		local lock_file=$(find_lock_file $board_no)
		local lock_user=$(get_lock_file_user $lock_file)
		if [ x"$lock_user" = x"$user" ]; then
			...
			echo "the board $board_no was locked by $lock_user($email)"
		...
		echo "the board $board_no is currently used by $curUser($email)"
		echo "if you really want to use it, please contact with him"
		return 1
	fi
}
